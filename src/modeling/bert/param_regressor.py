...
class Hand_Parameter_Regressor(nn.Module):
    ...
    def __init__(self):
        ...
        self.regressor = nn.Sequential(FCBlock(778*3, 1024),
                                        FCResBlock(1024, 1024),
                                        FCResBlock(1024, 1024),
                                        nn.Linear(1024, 10+3+3+45))
    
    def forward(self, pred_vertices):
        batch_size = pred_vertices.shape[0]
        x = pred_vertices.reshape(batch_size, -1)
        x = self.regressor(x)
        betas = x[:, :10]
        transl = x[:, 10:13]
        rot = x[:, 13:16]
        ...
        return betas, transl, rot, pose


class Face_Parameter_Regressor(nn.Module):
    """FLAME Parameter Regressor"""
    def __init__(self):
        super().__init__()
        self.regressor = nn.Sequential(FCBlock(1675*3, 1024),
                                        FCResBlock(1024, 1024),
                                        FCResBlock(1024, 1024),
                                        nn.Linear(1024, 100+50+6+3+3))
    ...
    def forward(self, pred_vertices):
        ...
        rot = x[:, 156:159]
        transl = x[:, 159:]
        return shape, exp, pose, rot, transl


class Hand_Pose_Regressor(nn.Module):
    """MANO Parameter Regressor"""
    def __init__(self):
        super().__init__()
        self.regressor = nn.Sequential(FCBlock(778*3, 1024),
...
class Face_Pose_Regressor(nn.Module):
    """FLAME Parameter Regressor"""
    def __init__(self):
        super().__init__()
        self.regressor = nn.Sequential(FCBlock(1675*3, 1024),
...
class Hand_Parameter_Regressor_Overfit(nn.Module):
    """MANO Parameter Regressor"""
    def __init__(self):
        super().__init__()
        self.regressor = nn.Sequential(FCBlock(778*3, 1024, batchnorm=False),
                                        FCResBlock(1024, 1024, batchnorm=False),
                                        FCResBlock(1024, 1024, batchnorm=False),
                                        nn.Linear(1024, 10+3+3+45))
...
class Face_Parameter_Regressor_Overfit(nn.Module):
    """FLAME Parameter Regressor"""
    def __init__(self):
        super().__init__()
        self.regressor = nn.Sequential(FCBlock(1675*3, 1024, batchnorm=False),
                                        FCResBlock(1024, 1024, batchnorm=False),
                                        FCResBlock(1024, 1024, batchnorm=False),
                                        nn.Linear(1024, 100+50+6+3+3))
